...
class GetListCurrentWeekTaskUseCase(private val repository: Repository):
    ...
    override suspend fun doWork(value: Boolean): DataResult<PaginatedScheduleResponseModel> {
        ...
        val startDate = today.with(DayOfWeek.MONDAY).atStartOfDay().format(DateTimeFormatter.ISO_LOCAL_DATE_TIME)
        val endDate = today.with(DayOfWeek.SUNDAY).atTime(LocalTime.MAX).format(DateTimeFormatter.ISO_LOCAL_DATE_TIME)
