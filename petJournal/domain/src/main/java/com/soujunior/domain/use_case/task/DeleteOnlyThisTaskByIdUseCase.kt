...
class DeleteOnlyThisTaskByIdUseCase(private val repository: Repository ) : BaseUseCase<String, Unit>() {
    override suspend fun doWork(value: String): DataResult<Unit> {
        return try {
            when (val response = repository.deleteOnlyThisTaskById(value)) {
