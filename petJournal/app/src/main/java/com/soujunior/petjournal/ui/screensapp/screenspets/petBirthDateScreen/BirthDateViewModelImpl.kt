...
class BirthDateViewModelImpl(
    ...
    override fun change(
        petBirth: String?,
        idPetInformation: Long?,
        petCastration: Boolean?,
    ) {
        when {
            petBirth != null -> {
                ...
                state =
                    if (result.success) {
                        state.copy(birthError = null)
                    } else {
                        state.copy(birthError = result.errorMessage)
                    }
            }
            ...
            petCastration == null || petCastration == true || petCastration == false -> {
                ...
                state =
                    if (result.success) {
                        state.copy(castrationError = null)
                    } else {
                        state.copy(castrationError = result.errorMessage)
                    }
            }
