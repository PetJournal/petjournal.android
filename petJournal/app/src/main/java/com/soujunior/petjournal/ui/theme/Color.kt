package com.soujunior.petjournal.ui.theme

import androidx.compose.ui.graphics.Color

//val primaryColor = Color(0xFF9A0963)
//val whiteBackgroundColor = Color(0xFFFFFFFF)

val light_primary = Color(0xFFB90063)
val light_onPrimary = Color(0xFFFFFFFF)
val light_secondary = Color(0xFFB90063)
val light_onSecondary = Color(0xFFFFFFFF)
val light_error = Color(0xFFBA1A1A)
val light_background = Color(0XFFFFFBFF)
val light_onBackground = Color(0xFF201A1B)
val light_surface = Color(0xFF8D8B8C)
val light_onSurface = Color(0xFF201A1B)
val light_onError = Color(0xFFFFFFFF)

val dark_primary = Color(0xFF99002C)
val dark_onPrimary = Color(0xFFFFFFFF)
val dark_primaryVariant = Color(0xFF790022)
val dark_secondary = Color(0xFF99002C)
val dark_onSecondary = Color(0xFFFFFFFF)
val dark_secondaryVariant = Color(0xFF790022)
val dark_error = Color(0xFFBA1A1A)
val dark_onError = Color(0xFFFFFFFF)
val dark_background = Color(0xFF121212)
val dark_onBackground = Color(0xFFFFFFFF)
val dark_surface = Color(0xFF222222)
val dark_onSurface = Color(0xFFFFFFFF)
