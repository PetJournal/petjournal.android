...
class PetBirthDateViewModelTest {
    ...
    fun `enable button when of the birth date and castration are validated`() {
        every { this@PetBirthDateViewModelTest.validation.validateDate(any()) } returns
            ValidationResult(
                success = true,
            )
        every { this@PetBirthDateViewModelTest.validation.validatePetCastration(any()) } returns
            ValidationResult(
                success = true,
            )
    ...
    fun `when change() is called with another birth date, should change the date`() {
        ...
        every { this@PetBirthDateViewModelTest.validation.validateDate(newBirth) } returns
            ValidationResult(
                success = true,
            )
