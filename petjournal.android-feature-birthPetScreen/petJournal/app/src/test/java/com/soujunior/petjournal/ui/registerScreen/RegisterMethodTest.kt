...
class RegisterMethodTest {
    ...
    @Test
    fun `phone number should have exactly 11 numbers`(){
        val phoneNumber = "21990399627"
        val event = RegisterFormEvent.PhoneChanged(phone = phoneNumber)
        every { this@RegisterMethodTest.validation.validatePhone(any()) } returns ValidationResult(
            success = true
        )
        viewModel.onEvent(event)
        assertEquals(viewModel.state.phone.length, phoneNumber.length)
        assertNull(viewModel.state.phoneError)
    }
}